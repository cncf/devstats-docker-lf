...
docker run --network=lfda_default --mount src="/data/devstats",target="/root",type=bind -e INIT=1 -e SKIPTEMP=1 -e NOLOCK=1 -e NOBACKUP=1 -e SKIPADDALL=1 -e UDROP="${UDROP}" -e LDROP="${LDROP}" -e DBDEBUG="${DBDEBUG}" -e ONLY="${ONLY}" -e GHA2DB_GITHUB_OAUTH="${GHA2DB_GITHUB_OAUTH}" -e GHA2DB_GHAPISKIP="${GHA2DB_GHAPISKIP}" -e PG_USER="${PG_USER}" -e PG_PORT="${PG_PORT}" -e TEST_SERVER=1 -e ES_PORT="${ES_PORT}" -e ES_HOST="${ES_HOST}" -e PG_HOST="${PG_HOST}" -e PG_PASS="${PG_PASS}" -e PG_PASS_RO="${PG_PASS_RO}" -e PG_PASS_TEAM="${PG_PASS_TEAM}" --env-file <(env | grep GHA2DB) devstats-lfda ./docker/docker_deploy_all.sh
